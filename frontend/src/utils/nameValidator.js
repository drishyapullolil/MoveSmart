/**
 * Validates a user's full name for card application, signup, and other forms.
 * Rules:
 ...
 * @returns {{ valid: boolean, reason: "missing_name" | "too_short" | "too_long" | "contains_spaces" | "contains_numbers" | "invalid_characters" | "valid", message: string, badgeType: "warning" | "error" | "success" }}
 */
export function validateName(name, fieldLabel = "Full Name") {
  ...
  if (/\s/.test(name)) {
    return {
      ...
      message: `${fieldLabel} cannot contain spaces. Use only alphabetic letters.`,
      badgeType: "error",
    };
  }
  ...
  if (/\d/.test(name)) {
    return {
      ...
      message: `${fieldLabel} cannot contain numbers. Use only alphabetic letters.`,
      badgeType: "error",
    };
  }
